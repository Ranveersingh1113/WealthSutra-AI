"""
WealthSutra AI — Backend API
=============================
FastAPI application that powers the WealthSutra AI financial advisor.

Responsibilities:
  - Serve the single-page frontend (fire_advisor_updated.html).
  - Calculate a user's financial health score across six categories.
  - Project a FIRE (Financial Independence, Retire Early) corpus using SIP math.
  - Proxy natural-language queries to the Groq LLM API for AI-powered advice.
...
app = FastAPI(
    title="WealthSutra AI",
    description="AI-powered Indian financial advisor API",
    version="1.0.0",
)
...
def open_browser():
    """Automatically open the default browser when the server starts up."""
